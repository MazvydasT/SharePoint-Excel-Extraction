...
export class SharePointService {
    ...
    getFilesInFolder<T>(folderURL: URL, restOptions?: IRESTOptions) {
        ...
        const filesRequestURL = new URL(`${folderURL.origin}${site}_api/web/GetFolderByServerRelativeUrl('${encodeURI(folderURL.pathname)}')/Files`);
        ...
        if (!!restOptions) {
            ...
            if (!!restOptions.orderby && restOptions.orderby.length > 0)
                searchParams.set(`$orderby`, toArray(from(restOptions.orderby).pipe(
                    mapIx(item => `${item.field} ${item.order ? Order[item.order] : ''}`.trim())
                )).join(`,`));

            if (!!restOptions.top)
                searchParams.set(`$top`, `${restOptions.top}`);
        }
    ...
    getLastAddedFileDataFromFolder(folderURL: URL) {
        return this.getFilesInFolder<ISharePointFilesData>(folderURL, {
            ...
            map(filesData => filesData.d.results.shift())
        );
    }
}